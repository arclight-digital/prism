...
/**
 * Deliberately unversioned, unlike the per-component headers.
 *
 * A wrapper file is rewritten whole on every run, so the version stamped in it
 * describes all of its content and a downgrade shows up as a changed first line.
 * A barrel is not: it is appended to and pruned in place, its header is written
 * once when the file is created, and its contents accrue across every version
 * that has ever touched the project. A version here would name the release that
 * happened to create the file and imply something about the lines below it that
 * isn't true.
 */
