...
/**
 * Deliberately unversioned, unlike the per-component headers. A wrapper is
 * rewritten whole, so its stamp describes all of its content; a barrel is
 * appended to and pruned in place, its contents accruing across every version
 * that has touched the project — a version here would name the release that
 * happened to create the file and imply something untrue about the lines below.
 */
 ...
 * matches newlines, so a statement a formatter has wrapped across lines reads
 * the same as a single-line one — which is why everything below works in
 * statements rather than lines; line-by-line matching silently skips every
 * wrapped export.
 */
 ...
 * is diff churn that gets flowed straight back on the next format.
 */
 ...
 *
 * The per-component updates above are append-only — right for watch and
 * single-file mode, which see one meta and must not judge the rest of the
 * barrel. The cost is that a deleted component's export lines survive forever
 * and the barrel stops compiling (TS2307) for whoever imports it. So unlike
 * `sweepOrphans` this is not gated behind --prune: an unbuildable barrel is a
 * bug, not untidiness.
 *
 * Decided by *resolution*, never by naming convention — rebuilding the
 * expected specifier set from the metas deletes working exports from barrels
 * prism doesn't generate (e.g. ones pointing at `./x.register.js`). Asking the
