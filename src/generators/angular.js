...
export function generateAngular(meta, config, root) {
  ...
  //
  // A prop whose name changes under lowercasing can't go through `[attr.x]` at
  // all: setAttribute lowercases, so `confirmLabel` lands as `confirmlabel`,
  // which Lit doesn't observe. Those bind as properties too. Kebab-case would be
  // worse — Lit's Boolean converter is presence-based, so `auto-resize="false"`
  // reads as true. Reflected props still show up as attributes, because Lit
  // reflects the property itself.
  const templateAttrs = meta.props.map((p) => {
    const isProperty = p.type === 'Boolean' || p.type === 'Array' || p.type === 'Object'
      || p.name !== p.name.toLowerCase();
    if (isProperty) return `[${p.name}]="this.${p.name}"`;
  ...
  // `<ng-content />` only where the element has a default slot to receive it.
  const slot = acceptsChildren(meta) ? '<ng-content />' : '';
  let template;
  if (templateAttrs.length > 0) {
    template = `<${meta.tag} ${templateAttrs.join(' ')}>${slot}</${meta.tag}>`;
  } else {
    template = `<${meta.tag}>${slot}</${meta.tag}>`;
  }
