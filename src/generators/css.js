...
import { HEADER_CSS as HEADER, isPrismGenerated, removeGenerated } from './header.js';
 ...
 * @param {{ prune?: boolean }} [opts] - actually delete stale output (default: report only)
 * @returns {{ results: Array<{ path: string, written: boolean }>, skipped: boolean, stale: string[], removed: string[] }}
 */
export function generateCSS(meta, config, root, { prune = false } = {}) {
  const outDir = join(root, config.outDir);
  const prefix = config.prefix;
  const fileName = meta.tag.replace(new RegExp('^' + prefix + '-'), '') + '.css';
  const outPath = join(outDir, fileName);

  // Interactive components get no CSS. The bundle already drops them, so a
  // per-component file left over from when this component was static would be
  // the only place those styles still ship.
  if (meta.interactivity === 'interactive') {
    const p = removeGenerated(outPath, { apply: prune });
    return {
      results: [],
      skipped: true,
      stale: p.stale ? [p.path] : [],
      removed: p.removed ? [p.path] : [],
    };
  }
  ...
  if (existsSync(outPath)) {
    ...
    if (!isPrismGenerated(existing)) {
      return { results: [{ path: outPath, written: false }], skipped: false, stale: [], removed: [] };
    }
  }
  ...
  return { results: [{ path: outPath, written: true }], skipped: false, stale: [], removed: [] };
}
