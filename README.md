...
`runtime` also answers for **methods**: `checkValidity()` contributed by a form-control mixin is a method of the element and of no file prism reads, and what a component's methods are is what decides whether its wrappers hand back a handle.

The prototype chain is walked to the mixin and no further. Where to stop is decided by **shape, not by name** — the prototype that owns `requestUpdate`, `performUpdate` and `createRenderRoot` is Lit's, whatever it is called. Names are not available to be tested: in the build a plain `import()` resolves, `LitElement.name` is `"i"` and `ReactiveElement.name` is `"g"`, so a name test passes against a dev build and fails in production, where it reports every component as driven by `enableUpdating()` and a mangled private. If the walk reaches the end of the chain without recognizing that boundary, it answers nothing and the component's own file is read instead, reporting `runtime-methods-unreadable` — a partial list is a smaller answer, but a list assembled past the boundary is a wrong one.
